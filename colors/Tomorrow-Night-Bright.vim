...
" 背景色为空是透明 Transparent，由 alacritty colors.primary.background 决定
let s:background = ""
let s:foreground = "#eaeaea" " white
let s:black000   = "#000000" " 为需要显示设置为 000 黑色的项使用

" selection 是选中背景
"let s:selection  = "#2C323C" " fg+bg-> special_grey
let s:selection  = "#282C34"
" line 是默认一行一行的默认背景
let s:line   = "#1E232A"
let s:comment    = "#7E8E91" " OK fg -> comment_grey
let s:window     = "#4d5057"
...
if has("gui_running") || &t_Co == 88 || &t_Co == 256
	...
	call <SID>X("TabLine", s:comment, s:black, "none")
	call <SID>X("TabLineFill", "", "", "none")
	call <SID>X("TabLineSel", "", "", "none")
	call <SID>X("StatusLine", s:comment, s:black, "none")
	call <SID>X("StatusLineNC", s:comment, s:selection, "none")
	call <SID>X("VertSplit", s:black, "", "none")
	...
	if version >= 700
		...
		call <SID>X("PMenu", s:blue, s:line, "none")
		...
		call <SID>X("SignColumn", "", s:black, "none")
	end
	if version >= 703
